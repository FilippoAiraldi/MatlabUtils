function [fillhandle, msg] = fill_between(x, ub, lb, color, edge, add, transparency)
    %USAGE: [fillhandle, msg] = fill_between(x, ub, lb, color, edge, add, transparency)
    ...
    %
    %John A. Bockstege November 2006; https://nl.mathworks.com/matlabcentral/fileexchange/13188-shade-area-between-two-curves
    %Example:
    ...
    if length(ub)==length(lb) && length(lb)==length(x)
        msg='';
        filled=[ub,fliplr(lb)];
        x=[x,fliplr(x)];
        if add
